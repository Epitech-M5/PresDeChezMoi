    ...
    const handleSubmit = async (event) => {
        ...
        if (selectedValue === 'Vente') {
            if (titre === null || prix === null || titre.length === 0 || prix.length === 0) {
                ...
                postAPI("http://127.0.0.1:8082/api/annonce/", { "titre": titre, "description": descriptions, 'image': image, 'dateDebut': dateDebut, "dateFin": dateFin, 'prix': prix, 'idTypeActivite': typeAct }, { "x-access-token": user.token })
                    .then(response => {
