...
const MySave = () => {
    ...
    const [idUserSave, setIdUserSave] = useState(null);
    ...
    useEffect(() => {
        getAPI('http://127.0.0.1:8081/api/user/', {}, { 'x-access-token': user.token })
            .then((response) => {
                ...
                setIdUserSave(response.dataAPI.enregistrements);
            })
            .catch((error) => {
                console.log('error', error);
                setLoading(false);
    ...
    const hasElementsToDisplay = reversedData.some((item) => saves.includes(item.id));
                    ...
                    {loading ? (
                        ...
                        <ul>
                            {hasElementsToDisplay ? (
                                ...
                                <h1>Vous n'avez pas encore enregistrer d'annonce !</h1>
                            )}
                        </ul>
                    )}
                </div>
