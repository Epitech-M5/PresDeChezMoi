...
const commentaireAdmin = db.commentaireAdmin;
...
exports.create = (req, res) => {
    ...
    // Create User
    const commentaireAdminObjet = {
        idUtilisateur: req.body.idUtilisateur,
        message: req.body.message
    };

    // Save Tutorial in the database adn catch internal error
    commentaireAdmin.create(commentaireAdminObjet)
        .then(data => {
            res.send(data);
        })
        .catch(err => {
...
exports.update = (req, res) => {
    ...
    commentaireAdmin.update(req.body, {
        where: { id: id }
    })
        .then(num => {
            if (num == 1) {
